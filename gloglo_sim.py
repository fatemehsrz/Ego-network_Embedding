...
def init_model():
    ...
    model.add(Dense(2000, input_dim=1100)) # 300+300+500
...
def run_network(data=None, model=None, epochs=20, batch= 32):
    try:
        ...
        csv_logger = CSVLogger('gloglo_sim.log')
...
################################# profile features reading

feature_file = open('common_feature.txt', "r")
...
for line in feature_file:
    ...
    for i in range(len(a1)):
        for j in range(len(a1[i])):
           if a1[i][j]== '0': Feature.append(0)
           if a1[i][j]== '1': Feature.append(1)
    #print(alter, Feature)   
    
    
    altersFeature.update({alter:Feature})  
    

############################### Vector concatenation both global vectors
...
for i in range(len(egoList)):       
    for j in egoCircleNodes[i]:
        ...
        #print('i:',i, 'j:',j)
        con1=np.concatenate((model1[str(egoList[i])], model1[str(j)]), axis=0) # glo glo sim
        con2=np.concatenate((con1, altersFeature[str(j)]), axis=0)
...
print(fscoreMain)    
print('local classifier F1 score: ',np.mean(fscoreMain)) 

print("--- %s seconds ---" % (time.time() - start_time))
   
    
#         
# f=open('gloglo_sim.fscore', 'w')
#     
