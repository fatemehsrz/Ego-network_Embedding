...
def init_model():
    ...
    model = Sequential()
    model.add(Dense(1400, input_dim=1400)) # 300+300+300+500
...
def run_network(data=None, model=None, epochs=20, batch= 32):
    try:
        ...
        csv_logger = CSVLogger('locgloglo_sim.log')
...
for i in range(len(egoList)):       
    for j in egoCircleNodes[i]:
        ...
        con2=np.concatenate((model1[str(j)], altersFeature[str(j)]), axis=0) # 
        con3=np.concatenate((con1, con2), axis=0)
        
        EgoAlterVectors.append(con3)
